...
const users_postgres_1 = __importDefault(require("../models/users-postgres"));
...
class AuthController {
    Register(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (yield users_postgres_1.default.isEmailAlreadyOnUse(email))
                return res.status(400).send("Email already on use");
            const newUserId = yield users_postgres_1.default.createNewUser({ username, email, password });
            if (newUserId == null)
    ...
    Login(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (users_postgres_1.default.getUserByEmail(email) == null)
                return res.status(400).send("not user with that email");
            const encryptedPassword = yield users_postgres_1.default.getEncryptedPassword(email);
            if (encryptedPassword == null)
                return res.status(500).send("Server error");
            if (!(yield bcrypt_1.default.compare(password, encryptedPassword)))
                return res.status(400).send("invalid credentials");
            const userData = yield users_postgres_1.default.getUserByEmail(email);
            if (userData == null)
                return res.status(500).send("Server error");
